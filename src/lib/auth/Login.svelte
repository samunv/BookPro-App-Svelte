<script>
	...
	async function iniciarSesion(event) {
		...
		try {
			const response = await fetch(
				"http://localhost/backend-bookpro/Controlador/procesarlogin.php",
				{
					method: "POST",
					body: datos,
				}
			...
			if (data.usuario) {
				...
				localStorage.setItem("foto", "http://localhost/backend-bookpro" + data.usuario.foto);
	...
	async function enviarCorreoRecuperacion() {
		try {
			const response = await fetch(
				`http://localhost/backend-bookpro/Controlador/procesarlogin.php?correoRecuperar=${correoRecuperar}`
			);
			const data = await response.json();
