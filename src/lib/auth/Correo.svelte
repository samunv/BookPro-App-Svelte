  ...
  async function verificarToken() {
    ...
    if (contadorError <= 2) {
      if (token === tokenAlmacenado) {
        ...
        alert("Token Incorrecto Vuelve a intentarlo");
        contadorError++;
      }
    } else {
      alert("Has fallado 2 veces, vuelve a introducir el correo.");
      cancelarRegistro();
    }
  }
...
<form class="formulario" on:submit={enviarCorreo}>
  <input
    type="email"
    id="correo"
    bind:value={correo}
...
{#if mostrarConfirmacion}
  <div id="seccion-confirmacion" transition:scale={{ duration: 200 }}>
    ...
    <form on:submit|preventDefault={verificarToken}>
      <input
        type="text"
        id="token"
        bind:value={token}
